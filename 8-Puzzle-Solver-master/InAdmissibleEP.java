			...
			while (!queue.isEmpty()){
				...
				if (!curState.getCurState().isGoal()){
					...
					for (int i = 0; i < candidates.size(); i++){
						State temp = null;
						//inadmissible heuristic, sum of inversions
						temp = new State(curState,candidates.get(i), curState.getACost()+ candidates.get(i).costs(),(candidates.get(i)).getSOI());
